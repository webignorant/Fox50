 ...
 * @author    liu21st <liu21st@gmail.com>
 */
class TagLibFhtml extends TagLib{
    // 标签定义
    ...
    public function _list($attr) {
        ...
        foreach($fields as $field) {//显示指定的字段
            ...
            if($sort) {
                //使用用户自定义的排序条件 $property[2]为自定义排序条件
                if(!isset($property[2])){
                    ...
                    $parseStr .= '<a href="javascript:sortBy(\''.$property[2].'\',\'<{$sort}>\',\''.ACTION_NAME.'\')" title="按照'.$showname[2].'<{$sortType}> ">'.$showname[0].'<eq name="order" value="'.$property[0].'" ><img src="../Public/images/<{$sortImg}>.gif" width="12" height="17" border="0" align="absmiddle"></eq></a></th>';
                }
