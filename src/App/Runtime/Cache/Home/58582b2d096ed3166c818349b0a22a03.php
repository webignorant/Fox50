...
<link rel="stylesheet" type="text/css" href="__PUBLIC__/Css/home_tag.css" />
<!-- 菜单区域  -->
    ...
    <div class="right_col">
        <div class="operate" >
            ...
            <form method='post' action="__URL__/index">
                <div class="fLeft"><span id="key"><input type="text" name="specialname" title="专辑查询" class="medium" ></span></div>
        ...
        <div class="list" >
            <!-- Think 系统列表组件开始 -->
<table id="checkList" class="list" cellpadding=0 cellspacing=0 ><tr><td height="5" colspan="4" class="topTd" ></td></tr><tr class="row" ><th width="8"><input type="checkbox" id="check" onclick="CheckAll('checkList')"></th><th><a href="javascript:sortBy('specialname','<?php echo ($sort); ?>','index')" title="按照专辑名称<?php echo ($sortType); ?> ">专辑名称<?php if(($order) == "specialname"): ?><img src="../Public/images/<?php echo ($sortImg); ?>.gif" width="12" height="17" border="0" align="absmiddle"><?php endif; ?></a></th><th><a href="javascript:sortBy('videonum','<?php echo ($sort); ?>','index')" title="按照专辑视频数<?php echo ($sortType); ?> ">专辑视频数<?php if(($order) == "videonum"): ?><img src="../Public/images/<?php echo ($sortImg); ?>.gif" width="12" height="17" border="0" align="absmiddle"><?php endif; ?></a></th><th><a href="javascript:sortBy('dateline','<?php echo ($sort); ?>','index')" title="按照创建时间<?php echo ($sortType); ?> ">创建时间<?php if(($order) == "dateline"): ?><img src="../Public/images/<?php echo ($sortImg); ?>.gif" width="12" height="17" border="0" align="absmiddle"><?php endif; ?></a></th></tr><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$videospecial): $mod = ($i % 2 );++$i;?><tr class="row" onmouseover="over(event)" onmouseout="out(event)" onclick="change(event)" ><td><input type="checkbox" name="key"	value="<?php echo ($videospecial["id"]); ?>"></td><td><?php echo ($videospecial["specialname"]); ?></td><td><?php echo ($videospecial["videonum"]); ?></td><td><?php echo (todate($videospecial["dateline"],'Y-m-d H#i#s')); ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?><tr><td height="5" colspan="4" class="bottomTd"></td></tr></table>
<!-- Think 系统列表组件结束 -->
...
function user_collect(){
    location.href  = '<?php echo U('Home/VideoFavorite/index');?>';
}
