<?php
//视频地区模块
class VideoRegionAction extends CommonAction {
    function _filter(&$map) {
        if(!empty($_POST['regionname'])) {
            $map['regionname'] = array('like',"%".$_POST['regionname']."%");
        }
    ...
    */
    ...
    public function checkName() {
        if($_POST['regionname'] == null ){
            $this->error( '视频地区名称不能为空！');
        }
        ///[^\u4e00-\u9fa5]/
        if(!preg_match('/^[\x7f-\xff]+$/',$_POST['regionname'])) {
            $this->error( '视频地区名称必须是中文！');
        }
        ...
        // 检测用户名是否冲突
        $name  =  $_REQUEST['regionname'];
        $result  =  $Region->getByRegionname($name);
        if($result) {
            $this->error('该名称已经存在！');
        }else {
            $this->success('该名称可以使用！');
