...
class NodeAction extends CommonAction {
    public function _filter(&$map){
        if(!empty($_GET['group_id'])) {
            ...
            $_SESSION['currentGroupId']	=	$map['group_id'];
            $this->assign('nodeName','分组');
        }elseif(empty($_POST['search']) && !isset($map['pid']) ) {
            $map['pid']	=	0;
        }else{
            $_SESSION['currentGroupId']	=	null;
        }
        if($_GET['pid']!=''){
            $map['pid']=$_GET['pid'];
        }
    ...
    public function sort()
    {
        ...
        if(!empty($_GET['sortId'])) {
            ...
            //分组情况下的排序
            if(!isset($pid)){
                 $sortList   =   $node->where('status=1 and group_id = '.$_SESSION['currentGroupId'])->order('sort asc')->select();
            }else{
