...
class CommonAction extends Action {
        ...
        if (C('USER_AUTH_ON') && !in_array(MODULE_NAME, explode(',', C('NOT_AUTH_MODULE')))) {
            ...
            if (!RBAC::AccessDecision()) {
                ...
                if (C('RBAC_ERROR_PAGE')) {
                    // 定义权限错误页面 redirect(C('RBAC_ERROR_PAGE'));
                    $this->error('该用户没有管理权限！','index.php/Public/logout/');
                } else {
                    if (C('GUEST_AUTH_ON')) {
    ...
    */
    ...
    /**
     *重写框架的display方法，让其能自动渲染top、footer页面
     */
    function display($templateFile='',$charset='',$contentType='',$content='',$prefix='') {
        //调用PublicAction里面的处理方法
        $public = A('Home/Public');
        $this->assign('NavList',$public->top());
        $this->assign('videoCategory',$public->top());
        $this->assign('FriendLink',$public->footer());
        //输出历史记录
        $recently = A('Home/VideoRecently');
        $this -> assign('RecentlyList', $recently -> showRecentlyOnHistoryDiv());
        //输出公告
        $notice = D('Notice');
        $this -> assign('notice',$notice->getNoticeRandomOnField('content'));
        //验证用户登录确定输出
        if(isset($_SESSION['username'])) {
            $this->assign('checklogin','true');
            $user = D('User');
            $map['account'] = $_SESSION['username'];
            $result = $user -> where($map) -> field('account,avatar') -> select();
